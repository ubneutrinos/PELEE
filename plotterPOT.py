    ...
    def _plot_variable_samples(self, variable, query, title, **plot_options):
        ...
        if "dirt" in self.samples:
            ...
            total_weight = np.concatenate(
                [ext_weight, mc_weight, nue_weight, dirt_weight])
        else:
            total_variable = np.concatenate(
        ...
        if "dirt" in self.samples:
            n_dirt, dirt_bins, patches = ax1.hist(
                ...
                bottom=n_mc + n_nue,
                weights=dirt_weight,
                label=r"Dirt: %.1f entries, pur = %.2f " % (sum(dirt_weight), sum(dirt_weight)/( sum(dirt_weight)+sum(nue_weight)+sum(mc_weight)+sum(ext_weight))),
                **plot_options)
