...
void tester_1e0p_unblinding(TString fSIDEBAND, TString fBLIND)
{
  ...
  for (auto i : ROOT::TSeqI(nentriesSIDEBAND)) {
    ...
    bool signal_open = (high_bdt && medium_energy);
    
    if (presel1e0p && signal_open) {

      far_sideband_SIDEBAND += 1;

      std::cout << "Run, Event "<< run << ", " << evt << " passes the 1e0p BDT selection for the (near+far) energy sideband in the SIDEBAND file." <<std::endl;
  ...
  for (auto i : ROOT::TSeqI(nentriesBLIND)) {
    ...
    if (presel1e0p && signal_open) {
      ...
      std::cout << "Run, Event " << run << ", " << evt << " passes the 1e0p BDT selection for the (near+far) energy sideband in the BLIND file." << std::endl;
